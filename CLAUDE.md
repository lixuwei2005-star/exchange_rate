...
| Alliance   | MYR per `Per Unit` foreign; `<table id="forexList">` header `Foreign Currency \| Country \| Per Unit \| Selling TT \| Selling Cash \| Buying TT \| Buying OD \| Buying Cash`; header & data rows have equal column count (no offset) | **CNY row `Buying TT` ÷ `Per Unit`** | `buying_tt / 100` |
| AmBank     | MYR per `Currency Unit` foreign; Sitefinity/Kendo `<table class="k-table">`, header is the red-bg `<tr>` of plain `<td>` cells `(flag) \| Currency \| Currency Unit \| Selling TT / OD (RM) \| Buying TT (RM) \| Buying OD (RM)`; header & data rows equal column count | **CNY row `Buying TT` ÷ `Currency Unit`** | `buying_tt / 100` |
| HSBC       | MYR per N units foreign; several server-rendered TT/Notes tables, the relevant header is `Currency \| Currency Name \| TT Sell \| TT Buy` and the **multiplier is in the table caption** (`… to 100 Units of Foreign Currency`), not a column. CNY is in the per-100 TT table; code is in the row `<th>`. Skip the Notes (cash) tables (no `TT Buy` header). | **CNY row `TT Buy` ÷ caption-multiplier** | `tt_buy / 100` |
| OCBC       | MYR per `Unit` foreign; one server-rendered table, header `Currency \| Unit \| Sell TT/OD \| Buy TT \| Buy OD`; each row's first cell is `CODE  NAME`. Board lives on the internet-banking **public** host (`internet.ocbc.com.my`), NOT `www` (Incapsula shell). Pick the **CNY** row, not the CNH (offshore) row. | **CNY row `Buy TT` ÷ `Unit`** | `buy_tt / 100` |
| Standard Chartered | MYR per `UNIT` foreign; FX board is one of ~17 server-rendered tables on the **deposits/interest-rates** page, header `UNIT \| CURRENCY \| SELLING TT/OD \| BUYINGTT \| BUYINGCP \| SWIFT CODE` (header labels arrive whitespace-collapsed — match on a space-stripped key). Imperva script referenced but does NOT block. | **CNY row `BUYINGTT` ÷ `UNIT`** | `buyingtt / 100` |
| Affin      | Static JSON feed `…/storage/rates/forex-100-unit.json`; CNY entry `{code:"(CNY)", buying:{tt,od}, selling:{ttod}}`. Per-100 feed. Affin publishes only **1 d.p.** (coarser than other banks — source precision). | entry `code=(CNY)` → **`buying.tt`** | `buying.tt / 100` |
...
| HSBC           | `hsbc`       | https://www.hsbc.com.my/investments/products/foreign-exchange/currency-rate/ (server-rendered HTML; plain GET with project UA returns 200, no bot wall. Several TT/Notes tables; multiplier is in the per-table caption, not a column — use the per-100 TT table's `TT Buy`) | 3 h     | Low |
| OCBC           | `ocbc`       | https://internet.ocbc.com.my/ocbc-public/ForeignExchangeTTODPublic/Index — the **internet-banking public host** (the `www.ocbc.com.my` page is an Incapsula JS shell; this host is plain HTML, project-UA GET → 200, no wall). One TT/OD table; pick the `CNY` row (not `CNH` offshore), `Buy TT ÷ Unit` | 3 h     | Low |
| Standard Chartered | `sc`     | https://www.sc.com/my/deposits/interest-rates/ (FX board is embedded here despite the deposits-y URL; project-UA GET → 200, Imperva script present but doesn't block). Header `UNIT \| CURRENCY \| SELLING TT/OD \| BUYINGTT \| BUYINGCP \| SWIFT CODE`; `BUYINGTT ÷ UNIT` | 3 h     | Low–Med — embedded table / Imperva |
| Affin          | `affin`      | https://www.affinalways.com/en/rates-and-pricing (JS carousel; we hit the static JSON it loads: `…/storage/rates/forex-100-unit.json`, project-UA GET → 200 JSON, no wall). `buying.tt ÷ 100`; only 1 d.p. published | 3 h     | Low |
...
5. Comparison table: sortable by "你能拿到". Columns: 渠道, 汇率 (CNY per MYR for display), 手续费, 你能拿到 (MYR), 更新于. Stale rows are **kept visible** (greyed out, value shown as "暂时不可用") — never dropped, so a channel never silently disappears. The 更新于 column shows `rate_changed_at` (when the rate VALUE last changed), not the last poll time, so a rate that hasn't moved doesn't appear to refresh every tick. `is_stale` is computed in `api/public.py:_stale_threshold_for(channel)` **from the channel's schedule** (interval → 2× interval ≥ 12 h; daily → 30 h; weekly → max weekday-gap + 1 d + 6 h grace), NOT a hardcoded per-code table — so any channel the admin switches to daily/weekly automatically gets a window wide enough to survive the gap between fires.
