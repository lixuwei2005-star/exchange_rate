...
用户会提供今日各渠道（中国银行、银联、Visa、Wise、马来西亚各银行等）的汇率，以及近 7 日 / 30 日走势。汇率以「1 MYR = X CNY」表示：数值越低，同样的人民币能换到越多马币，越划算。
请用一句话（不超过 80 字）总结：今日哪个渠道最划算、与中间市场参考价的差距，以及人民币近 7 日 / 30 日的涨跌趋势。
只陈述事实与数字，不给出任何换汇时机建议，不使用"赶紧""立刻""建议换汇"等催促性措辞；不使用表情符号；语气平实简洁。"""
